...
results = {
    'Model': ['Pure CNN', 'DistilBERT (Optimized)'],
    'Test Accuracy': [0.6200, 0.6289],
    'Test Macro-F1': [np.nan, 0.6125],
    'Validation Accuracy': [0.6214, 0.6251],
    'Validation Macro-F1': [np.nan, 0.6115],
    'Training Time (approx)': ['~45 min', '~90 min'],
    'Parameters': ['792,326', '67,217,026'],
    'Architecture': ['Multi-kernel CNN (3,5,7)', 'DistilBERT + Dense Head'],
}
...
models = ['Pure CNN', 'DistilBERT']
accuracy = [0.6200, 0.6289]
macro_f1 = [np.nan, 0.6125]

x = np.arange(len(models))
width = 0.35

bars1 = ax.bar(x - width/2, accuracy, width, label='Test Accuracy', alpha=0.8, color='#1f77b4')
bars2 = ax.bar(x + width/2, [np.nan, 0.6125], width, label='Macro-F1', alpha=0.8, color='#ff7f0e')
